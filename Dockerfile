...
ENV DEBIAN_FRONTEND=noninteractive \
    PATH=/opt/cinc-workstation/bin:/opt/cinc-workstation/embedded/bin:/root/.chefdk/gem/ruby/2.6.0/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
...
RUN apt-get update && \
    ...
    wget --content-disposition "http://downloads.cinc.sh/files/${CHANNEL}/cinc-workstation/${VERSION}/ubuntu/18.04/cinc-workstation_${VERSION}-1_amd64.deb" -O /tmp/cinc-workstation.deb && \
